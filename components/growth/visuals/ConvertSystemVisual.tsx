...
export function ConvertSystemVisual() {
  return (
    <div className="relative w-full h-72 lg:h-80 overflow-hidden rounded-xl border border-charcoal-200 bg-white flex flex-col items-center justify-center p-4 lg:p-6">
      {/* Grid Background */}
      ...
      <div className="relative z-10 flex flex-col items-center gap-1.5 lg:gap-2 w-full max-w-sm">
        {FUNNEL_STAGES.map((stage, i) => (
          <div key={stage.label} className="relative flex flex-col items-center w-full group">
            {/* Funnel Block */}
            <div 
              className={cn(
                "h-8 lg:h-10 rounded-sm flex items-center justify-center transition-all duration-500 border border-charcoal-200/50",
                stage.color,
                stage.textColor
              )}
              style={{ width: stage.width }}
            >
              <span className="text-[9px] lg:text-xs font-bold tracking-wide uppercase whitespace-nowrap px-2">{stage.label}</span>
            </div>
            
            {/* Connector */}
            {i < FUNNEL_STAGES.length - 1 && (
              <div className="h-3 lg:h-4 flex items-center justify-center">
